...
public class ViewController {
    ...
    public String prenotazioneSubmit(
            @RequestParam Long localeId,
        ...
        try {
            ...
            prenotazione.setLocale(localeService.findById(localeId));
            prenotazione.setGioco(giocoService.findById(giocoId));
            prenotazione.setDataOra(LocalDateTime.parse(dataOra));
            if (userDetails != null) {
                prenotazione.setUtente(utenteService.findByUsername(userDetails.getUsername()));
            }
            prenotazioneService.crea(prenotazione);
            redirectAttributes.addFlashAttribute("success", "Prenotazione effettuata con successo!");
            return "redirect:/prenotazione?success";
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("errore", e.getMessage());
    ...
    public String adminUtenti(@AuthenticationPrincipal UserDetails userDetails, Model model) {
        model.addAttribute("utenti", utenteService.findAll());
        model.addAttribute("utenteCorrente", utenteService.findByUsername(userDetails.getUsername()));
        return "admin/utenti";
    }

    @PostMapping("/admin/utenti/{id}/ruolo")
    public String cambiaRuoloUtente(@PathVariable Long id,
                                    @RequestParam String ruolo,
                                    @AuthenticationPrincipal UserDetails userDetails,
                                    RedirectAttributes redirectAttributes) {
        try {
            Utente corrente = utenteService.findByUsername(userDetails.getUsername());
            if (corrente.getId().equals(id)) {
                throw new RuntimeException("Non puoi cambiare il ruolo del tuo stesso account da qui.");
            }
            utenteService.cambiaRuolo(id, Utente.RuoloUtente.valueOf(ruolo));
            redirectAttributes.addFlashAttribute("success", "Ruolo aggiornato.");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("errore", e.getMessage());
        }
    ...
    public String eliminaUtente(@PathVariable Long id,
        ...
        try {
            Utente corrente = utenteService.findByUsername(userDetails.getUsername());
            if (corrente.getId().equals(id)) {
                throw new RuntimeException("Non puoi eliminare il tuo stesso account da qui.");
            }
            utenteService.delete(id);
            redirectAttributes.addFlashAttribute("success", "Utente eliminato.");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("errore",
                    "Impossibile eliminare l'utente: probabilmente ha partite, prenotazioni o un locale collegati.");
        }
        return "redirect:/admin/utenti";
    }
    ...
    public String nuovaPartitaForm(Model model) {
        model.addAttribute("giochi", giocoService.findAll());
        model.addAttribute("utenti", utenteService.findByRuolo(Utente.RuoloUtente.GIOCATORE));
    ...
    public String nuovaPartita(
        ...
        try {
            ...
            if (giocatoriIds != null && !giocatoriIds.isEmpty()) {
                List<Utente> giocatori = giocatoriIds.stream()
                        .map(utenteService::findById)
